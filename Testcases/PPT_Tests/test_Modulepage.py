...
class Test_Module:
    ...
    def test_CreateNew_Module(self, user_login):
        ...
        rand_newcavity_no = ''.join((random.choice('123') for i in range(2)))
        self.projprop.setNewModuleNo_Cavity(rand_newcavity_no)
        rand_newswitch_no = ''.join((random.choice('12345') for i in range(1)))
    ...
    def test_Module_Prop(self, user_login):
        ...
        time.sleep(2)
        mprop = self.driver.find_elements(By.XPATH, "/html/body/div[1]/div[2]/main/div[3]/div/div[3]/div/div/div[1]/span[2]/div/div/div[2]/div")
        print(len(mprop))
        expected = ['Description', 'Module Board COMM Pin', 'Number of Cavity', 'Number of Switch', 'Module Cavity LED No']
        contents = []
        for mp in mprop:
